pipeline {
    ...
    stages {
        ...
        stage('Test') {
            steps {
                sh 'docker run -it --rm -v "$PWD":/var/jenkins_home/workspace/pythontespipeline -w /var/jenkins_home/workspace/pythontespipeline/ qnib/pytest py.test --verbose --junit-xml test-reports/results.xml sources/test_calc.py'
            }
            post {
                always {
